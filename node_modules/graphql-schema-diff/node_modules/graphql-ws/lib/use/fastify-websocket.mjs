...
keepAlive = 12000) {
    ...
    return (connection, request) => {
        ...
        socket.on('error', (err) => socket.close(CloseCode.InternalServerError, isProd ? 'Internal server error' : err.message));
        ...
        const closed = server.opened({
            ...
            onMessage: (cb) => socket.on('message', async (event) => {
                try {
                    ...
                    socket.close(CloseCode.InternalServerError, isProd ? 'Internal server error' : err.message);
                }
            }),
        }, { connection, request });
